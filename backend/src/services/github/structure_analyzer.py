...
class StructureAnalyzer:
    ...
    def analyze(cls, repo_path: str) -> Dict[str, Any]:
        ...
        for root, dirs, files in os.walk(repo_path):
            ...
            # Filter directories using ExclusionEngine
            dirs[:] = [d for d in dirs if not ExclusionEngine.is_excluded(os.path.join(rel_path, d))]
            
            structure["directories"].append(rel_path)
            
            for file in files:
                file_rel_path = os.path.join(rel_path, file)
                if ExclusionEngine.is_excluded(file_rel_path):
                    continue
